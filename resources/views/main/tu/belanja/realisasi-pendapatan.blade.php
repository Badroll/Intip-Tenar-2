                ...
                @if(count($ctl_data) > 0)
                    @foreach($ctl_data as $aData)
                        <tr>
                            ...
                            <td>
                                <input type="text" class="form-control numeric-input-idr" value="{{ $aData->PNP_JUMLAH }}" id="jumlah_{{ $aData->PNP_ID }}">
                            </td>
                        </tr>
                    @endforeach
                @else
...
<script type="text/javascript">
    ...
    function simpan(id){
        ...
        for (var i = 0; i < gArrId.length; i++){
            ...
            var data = {
                PNP_ID : id,
                PNP_JUMLAH : $("#jumlah_" + id).autoNumeric("get")
            };
            if(
                data.PNP_JUMLAH.trim() == ""
                ){
